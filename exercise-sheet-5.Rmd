...
Wir betrachten eine Kette mit \( N \) Massepunkten, die durch \( N-1 \) Federn verbunden sind. Die Massepunkte haben eine Masse \( m \) und sind in den Positionen \( (y_i, z_i) \), mit \( i = 0, \ldots, N \). Die beiden äußersten Massepunkte sind fixiert bei \( (y_0, z_0) = (-2, 1) \) und \( (y_N, z_N) = (2, 1) \).

Wir wollen eine Ruheposition der Kette finden, was der Minimierung der Kettenenergie \( V(y, z) \) entspricht. Diese setzt sich zusammen aus der Lageenergie der Massen und der potenziellen Energie der Federn:

\[
V(y, z) = \sum_{i=0}^{N} m g z_i + \frac{1}{2} \sum_{i=0}^{N-1} D \left( (y_{i+1} - y_i)^2 + (z_{i+1} - z_i)^2 \right),
\]

wobei \( g \) die Erdbeschleunigung und \( D \) die Federkonstante ist.

Zusätzlich wird angenommen, dass unterhalb der Kette eine ebene Fläche liegt, die die Kette nach unten begrenzt. Dies wird durch die Nebenbedingung \( z_i \geq 0 \) ausgedrückt.

Das Problem wird als nichtlineares Optimierungsproblem (NLP) formuliert:

\[
\begin{aligned}
\min \quad & V(y, z) \\
\text{s.t.} \quad & (y_0, z_0) = (-2, 1), \\
& (y_N, z_N) = (2, 1), \\
& z_i \geq 0, \quad \forall i = 1, \ldots, N
\end{aligned}
\]

Dieses NLP ist konvex aufgrund der linearen Nebenbedingungen \( z_i \geq 0 \).

Eine Implementierung dieses Problems ist in der Datei `chain.py` zu finden, zusammen mit einer grafischen Darstellung der Lösung in Abbildung 2.
